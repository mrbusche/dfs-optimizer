...
def test_only_use_players():
    ...
    try:
        ...
        only_use = ["Lamar Jackson", "Josh Allen",  # QBs
...
def test_missing_only_use_player_warning(capsys):
    ...
    try:
        ...
        only_use = ["NonExistentPlayer456", "Lamar Jackson", "Josh Allen",
...
def test_position_constraints():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            assert player_count == expected_player_count, f"Expected {expected_player_count} players, found {player_count}"
    finally:
        if os.path.exists(output_file + '.csv'):
            os.remove(output_file + '.csv')
